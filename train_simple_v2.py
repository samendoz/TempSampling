...
if args.profile_to_dgl_blocks:
    ...
    set_mailbox_prep_profile(True, AGGRESSIVE_PROFILING)
...
if args.mode == "observing" or args.mode == "batch_stable_freezing" or args.mode == "batch_stable_freezing_large" or args.mode == "observing_large":
    ...
    if args.sampler_for_each_chunk:
        ...
        multi_sampler_cls = GPUMultiColorBatchSampler if args.use_gpu_color_sampler else MultiColorBatchSampler
        multi_sampler_kwargs = dict(num_colors=NUM_COLORS,
                                     num_recent_edges=args.window_size,
                                     num_hops=args.color_num_hops,
                                     num_workers=args.color_num_workers,
                                     num_threads_per_worker=args.color_num_threads_per_worker,
        ...
        if args.use_gpu_color_sampler:
            multi_sampler_kwargs['device'] = 'cuda'
        else:
            multi_sampler_kwargs['use_nogil'] = args.use_nogil_sampling
        color_sampler = multi_sampler_cls(indptr, edge_index, indices, eid, num_nodes, num_edges,
                                           **multi_sampler_kwargs)
    else:
        print("use original color batch sampler...")
        sampler_cls = GPUColorBatchSampler if args.use_gpu_color_sampler else ColorBatchSampler
        sampler_kwargs = dict(num_colors=NUM_COLORS,
                               num_recent_edges=args.window_size,
                               num_hops=args.color_num_hops,
                               num_workers=args.color_num_workers,
                               num_threads_per_worker=args.color_num_threads_per_worker,
        ...
        if args.use_gpu_color_sampler:
            ...
            sampler_kwargs['use_nogil'] = args.use_nogil_sampling
        color_sampler = sampler_cls(indptr, edge_index, indices, eid, num_nodes, num_edges,
                                     **sampler_kwargs)
...
for e in range(train_param['epoch']):
    ...
    if args.mode == 'batch_stable_freezing':
        ...
        print("\testimated initial to_dgl_blocks time: {:.2f}s, prepare_input time: {:.2f}s, mailbox prep time: {:.2f}s, post to_dgl_blocks time: {:.2f}s, mailbox update time: {:.2f}s, update_memory_and_check_stablizing time: {:.2f}s, updating indptr and stable flag time: {:.2f}s, get_stable_flag time: {:.2f}s, edge_feat_index time: {:.2f}s".format(
            ...
            estimated_prep_times["edge_feat_index"]
        ))
